import React from 'react'
import { Link } from 'react-router-dom'

const UserCard = ({user, isConnection}) => {
    // Add safety check for user data
    if (!user) {
        console.error("UserCard: user prop is undefined");
        return <div className='bg-white rounded-lg shadow p-4'>Invalid user data</div>;
    }

    // Safely get connections count
    const connectionsCount = user.connections?.length || 0;

    return (
        <div className='bg-white rounded-lg shadow p-4 flex flex-col items-center transition-all hover:shadow-md'>
            <Link to={`/profile/${user.username || 'unknown'}`} className='flex flex-col items-center'>
                <img 
                    src={user.profilePicture || "/avatar.png"} 
                    alt={user.name || "User"}
                    className='w-24 h-24 rounded-full object-cover mb-4' 
                />
                <h3 className='font-semibold text-lg text-center'>
                    {user.name || "Unknown User"}
                </h3>
            </Link>

            <p className='text-gray-600 text-center'>
                {user.headline || "LinkedIn User"}
            </p>
            <p className='text-sm text-gray-500 mt-2'>
                {connectionsCount} connections
            </p>

            <button className='mt-4 bg-primary text-white px-4 py-2 rounded-md hover:bg-primary-dark transition-colors w-full'>
                {isConnection ? "Connected" : "Connect"}
            </button>
        </div>
    )
}

export default UserCard